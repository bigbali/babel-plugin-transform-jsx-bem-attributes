...
const buildValue = (block, elem, mods, element, isBlockInherited) => {
    ...
    const templatePrefix = (!isBlockInherited && elem && index_1.OPTIONS.block.preserve)
        ? `BLOCK${exports.SPACE}'TEMPLATE_PREFIX'`
        : 'TEMPLATE_PREFIX';
    ...
    if (core_1.types.isObjectExpression(mods)) {
        mods.properties.forEach((property, index) => {
            ...
            if (core_1.types.isObjectProperty(property)) {
                ...
                template.quasis.push(core_1.types.templateElement({ raw: 'QUASI' }, index === Object.keys(mods.properties).length));
            }
        });
    }
...
const constructClassNameAttribute = (BEM_PROPS, ELEMENT, IS_BLOCK_INHERITED) => {
    ...
    const attributeValue = (() => {
        ...
        if (typeof final === 'string') {
            ...
            return core_1.types.stringLiteral(final + ' FINAL_VALUE_DETERMINED');
        }
    })();
    if (!attributeValue)
        return null;
    return core_1.types.jsxAttribute(core_1.types.jsxIdentifier('classNameWTF'), attributeValue);
};
