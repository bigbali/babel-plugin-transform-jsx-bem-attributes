...
const buildValue = (block, elem, mods, element, isBlockInherited) => {
    ...
    const templatePrefix = (!isBlockInherited && elem && index_1.OPTIONS.block.preserve)
        ? `${exports.SPACE}${prefix}`
        : prefix;
    const template = core_1.types.templateLiteral([
        core_1.types.templateElement({ raw: templatePrefix }, false)
    ], []);
        ...
        mods.properties.forEach((property, index) => {
            ...
            if (core_1.types.isObjectProperty(property)) {
                ...
                if (core_1.types.isRestElement(property.value))
                    return;
                template.expressions.push(core_1.types.callExpression(core_1.types.arrowFunctionExpression([], core_1.types.callExpression(core_1.types.memberExpression(core_1.types.identifier('console'), core_1.types.identifier('log')), [core_1.types.stringLiteral(key), property.value])), []));
                template.quasis.push(core_1.types.templateElement({ raw: '' }, false));
                template.expressions.push(core_1.types.conditionalExpression(property.value, core_1.types.stringLiteral(`${exports.SPACE}${prefix}${constants_1.MODS_CONNECTOR}${key}`), core_1.types.stringLiteral('')));
                template.quasis.push(core_1.types.templateElement({ raw: '' }, index === Object.keys(mods.properties).length));
            }
        });
    }
